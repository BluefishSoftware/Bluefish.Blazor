...
public static class IQueryableExtensions
{
    public static IQueryable<T> ApplyFilter<T>(this IQueryable<T> query, Filter filter, IDictionary<string, string> keyProperyMappings = null)
    {
        try
        {
            ...
            {
                object[] parameters = filter.Type switch
                {
                    FilterTypes.In => filter.Values.Select(x => x.RemoveQuotes()).ToArray(),
                    FilterTypes.Range => new[] { filter.Values[0].RemoveQuotes(), filter.Values[1].RemoveQuotes() },
                    FilterTypes.IsEmpty => new[] { string.Empty },
                    FilterTypes.IsNotEmpty => new[] { string.Empty },
                    _ => new object[] { filter.Values[0].RemoveQuotes() }
                };
                var predicate = filter.Type switch
                {
